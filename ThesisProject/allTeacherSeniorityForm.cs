        ...
        private void lecturerSeniorityScheck()
        {
            try
            {
                ...
                try
                {
                    ...
                    cmd = new SqlCommand();
                    cmd = connection.CreateCommand();
                    cmd.CommandText = "Select name, discipline_name , joining_date From info,joining,discipline where joining.designation_id = '1' AND joining.teacher_id = info.Id AND joining.current_designation='1' AND info.discipline = discipline.discipline_id ORDER BY active DESC";
